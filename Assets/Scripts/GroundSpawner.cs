...
public class GroundSpawner : MonoBehaviour
{
    ...
    public void SpawnTile(bool spawnItems) //Yolun oluşuturulması.
    {
        ...
        if(spawnItems)
        {
            temp.GetComponent<GroundTile>().SpawnObstacle(); //Tuzaklarin olusturulmasi.
    ...
    private void Start()
    {
        GroundTile.zombiePrefab = 1; // karakterin yoldan çıktığın ileriye zombilerin eklenmesini sayar.
        GroundTile.tileExit = 5; // karakterin yoldan çıkmasını sayar.
        GroundTile.zombieDead = 0; //ilerledikce zombiler ölür.
        ...
        for (int i = 0; i < 30; i++)
        {
            if (i < 3) //Ilk yolda tuzak olmamasi.
            {
                SpawnTile(false);//tuzaklar basılmıyor.
            }
            else
            {
                SpawnTile(true);//tuzaklar olusturuluyor.
